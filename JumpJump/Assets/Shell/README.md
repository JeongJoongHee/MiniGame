...
미니게임 화면 **오른쪽 맨 위**에 붙는 되돌아가기 버튼입니다.
게임 중에도 항상 보이고, 아무 때나 눌러 로비로 나올 수 있습니다.

**게임마다 따로 만들지 않습니다.** `ShellSceneBuilder.MakeLobbyButton(root, round, hud)` 이
한 곳에서 만들기 때문에 어느 게임에서나 자리와 모양이 같습니다.
새 미니게임을 붙일 때도 그 한 줄만 부르면 되고, HUD 쪽에는 `OnLobbyPressed()` 만 있으면 됩니다.
...
- **설정(톱니바퀴) 버튼은 없습니다.** 예전 로비 그림에는 오른쪽 위에 그려져 있었지만
  동작하지 않는 장식이라 2026-09-07 에 그림에서 지웠습니다. 톱니바퀴가 있는 원본은
  작업 폴더의 `Lobby_원본_톱니바퀴포함.png` 로 남겨 두었습니다.
